...
export default async function SummaryPage({ params, searchParams }: PageProps) {
  ...
  const metric = (row: any, key: string) => key === "team" ? row.team?.team_number : key === "rank" ? asNumber(row.tba?.rank || 99999) : key in row.stats ? asNumber(row.stats[key]) : asNumber(row[key]); const sort = query.sort ?? "rank"; const dir = query.dir === "desc" ? "desc" : "asc"; rows.sort((a: any, b: any) => { const diff = metric(a, sort) - metric(b, sort); return diff * (dir === "asc" ? 1 : -1); });
