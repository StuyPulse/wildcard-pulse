...
export default async function MatchesPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const [{ data: matches }, { data: eventTeams }, { data: membership }] = await Promise.all([
    supabase.from("matches").select("id,tba_match_key,match_number,match_type,red_teams,blue_teams,scheduled_at,status").eq("event_id", eventId).order("scheduled_at"),
    supabase.from("event_teams").select("team_id,teams(team_number)").eq("event_id", eventId),
  ...
  return <AppShell active="Matches"><PageHeader eyebrow="Event schedule" title="Matches." />
    {membership && <section className="card" style={{ marginBottom: 18 }}><div className="card-head"><div><h2>Activate scouting</h2><p className="muted" style={{ margin: "6px 0 0" }}>After importing, create assignments so scouts can open their forms.</p></div><Link className="button secondary" href={`/admin/assignments?eventId=${eventId}`}>Assignment board</Link></div><GenerateAssignmentsForm eventId={eventId} /></section>}
    <section className="card"><div className="card-head"><h2>Match schedule</h2><Link className="button secondary" href={`/events/${eventId}/teams`}>Team directory</Link></div>{matches?.length ? matches.map((match) => <div className="match-row" key={match.id}><span className="match-time">{match.scheduled_at ? new Date(match.scheduled_at).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) : "—"}</span><span className="match-num">{displayMatch(match.tba_match_key, match.match_number, match.match_type)}</span><div className="alliances"><div className="red">{displayAlliance(match.red_teams)}</div><div className="blue">{displayAlliance(match.blue_teams)}</div></div><span className="tag pending">{match.status}</span></div>) : <p className="muted">The schedule will appear after the event is imported.</p>}</section>
  </AppShell>;
}
