...
export default async function MatchesPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return <AppShell active="Matches"><PageHeader eyebrow="Event schedule" title="Matches." />
    {membership && <section className="card" style={{ marginBottom: 18 }}><div className="card-head"><div><h2>Activate scouting</h2><p className="muted" style={{ margin: "6px 0 0" }}>After importing, create assignments so scouts can open their forms.</p></div></div><GenerateAssignmentsForm eventId={eventId} /></section>}
