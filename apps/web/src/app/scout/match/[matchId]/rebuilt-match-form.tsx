...
const spots = [
  ...
  { id: "depot", label: "Depot", x: "18%", y: "10%" },
  { id: "depot-bump", label: "Depot Bump", x: "18%", y: "30%" },
  { id: "hub", label: "Hub", x: "18%", y: "50%" },
  { id: "outpost-bump", label: "Outpost Bump", x: "18%", y: "70%" },
  { id: "outpost", label: "Outpost", x: "18%", y: "90%" },
...
export function RebuiltMatchForm({ eventId, matchId, teamId, assignmentId, teamNumber, alliance = "red", otherTeams, manualMatch }: Props) {
  ...
  return <section className="scouting-card match-form">
    <div className="form-intro"><div className="form-kicker">{manualMatch ? `Manual match report · ${manualMatch.stage}${manualMatch.label ? ` · ${manualMatch.label}` : ""}` : `Match scouting · ${alliance} alliance`}</div><h2>Team {teamNumber}</h2><p>{manualMatch ? "This exception uses the same match-scouting fields and saves to the same team history as scheduled reports." : "Use the large controls while the match runs. Save a draft at any point; submit once the report is complete."}</p></div>
      <div className="form-section"><div className="section-title">Auton starting position</div><p className="muted">The red labels run Line 1 → Depot → Depot Bump → Hub → Outpost Bump → Outpost → Line 2; blue mirrors that order. On a phone, tap a position once to select it and again to confirm it.</p><button type="button" className="button secondary mobile-full" disabled={saving || submitted} aria-pressed={noShow} onClick={() => setNoShow(!noShow)}>{noShow ? "Undo no show" : "Mark no show"}</button><fieldset disabled={disabled}><legend className="sr-only">Autonomous starting position</legend><div className={`field-map ${alliance === "blue" ? "flipped" : "red-side"}`}><div className="field-map-art" aria-hidden="true"/>{spots.map((item) => <button type="button" key={item.id} aria-label={`Start at ${item.label}`} aria-pressed={spot === item.id} style={{"--spot-x":item.x,"--spot-y":item.y} as CSSProperties} className={spot === item.id ? `spot ${alliance}${spotConfirmed ? " confirmed" : ""}` : "spot"} onClick={() => { setSpot((current) => current === item.id ? undefined : item.id); setSpotConfirmed(false); }} onDoubleClick={() => { setSpot(item.id); setSpotConfirmed(true); }}><span>{item.label}</span></button>)}</div><div className="mobile-start-spots">{spots.map((item) => <button type="button" key={item.id} aria-pressed={spot === item.id} className={spot === item.id ? `spot-mobile ${alliance}${spotConfirmed ? " confirmed" : ""}` : "spot-mobile"} onClick={() => { if (spot === item.id) setSpotConfirmed(true); else { setSpot(item.id); setSpotConfirmed(false); } }}>{item.label}{spot === item.id && <span>{spotConfirmed ? " · confirmed" : " · tap to confirm"}</span>}</button>)}</div></fieldset><div className="spot-choice" aria-live="polite">{spot ? `Starting position: ${spots.find((item)=>item.id===spot)?.label}${spotConfirmed ? " · confirmed" : ""}` : "Choose one of 7 starting positions."}</div></div>
