...
export function ManualScouting({ eventId, teams, type = "pre_scout" }: { eventId: string; teams: Team[]; type?: EntryType }) {
  ...
  const matchingTeams = sorted.filter((team) => `${team.number} ${team.name}`.toLowerCase().includes(teamQuery.trim().toLowerCase()));
